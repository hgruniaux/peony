...
p_line_map_init(PLineMap* p_lm)
{
  ...
  p_lm->positions = static_cast<uint32_t*>(malloc(sizeof(uint32_t) * INITIAL_CAPACITY));
  assert(p_lm->positions != nullptr);
}
...
p_line_map_add(PLineMap* p_lm, uint32_t p_line_pos)
{
  ...
  if (p_lm->size == p_lm->capacity) {
    ...
    p_lm->positions = static_cast<uint32_t*>(realloc(p_lm->positions, sizeof(uint32_t) * p_lm->capacity));
    assert(p_lm->positions != nullptr);
  }
...
{
  assert(p_lm != nullptr && p_lm->positions != nullptr);
...
p_line_map_get_lineno(PLineMap* p_lm, uint32_t p_position)
{
  ...
  uint32_t linepos;
  p_line_map_get_lineno_and_colno(p_lm, p_position, &linepos, nullptr);
  return linepos;
}
...
p_line_map_get_colno(PLineMap* p_lm, uint32_t p_position)
{
  ...
  uint32_t colno;
  p_line_map_get_lineno_and_colno(p_lm, p_position, nullptr, &colno);
  return colno;
}
